...
def main():
    ...
    read_data_for_geo_level_into_database(
        directory, geo_levels_to_read, disk_engine)
...
def get_column_list_table_name_array(table_name, column_list, long_header_lookup):
    ...
    return map(lambda column_name:
               [long_header_lookup.get_value(column_name, 'Long'), column_name, table_name], column_list)
...
def read_data_for_geo_level_into_database(directory, geo_levels, disk_engine):
    ...
    for geo_level in geo_levels:
        ...
        long_header_lookup = pandas.read_excel(directory + '/../Metadata/Metadata_2016_GCP_DataPack.xls',
                                               sheetname='Cell descriptors information', header=10, index_col=1)
        long_header_lookup = long_header_lookup[~long_header_lookup.index.duplicated(
            keep='first')]
        ...
        for i, filename in enumerate(directory_file_list):
            ...
            if table_name_with_geo_level not in table_names:
                ...
                df.to_sql(table_name_with_geo_level,
                          disk_engine, if_exists='fail')
                column_list = list(df)
            else:
                column_list = get_column_names_from_table(
                    disk_engine, table_name_with_geo_level)
...
def update_metadata(directory, disk_engine):
    table_names = get_table_names_from_database(disk_engine)
    progress_bar = progressbar.ProgressBar(maxval=len(table_names),
                                           widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
    ...
    if "metadata" in table_names:
        table_columns_df = pandas.read_sql(
            "SELECT * FROM metadata", connection)
    ...
    for i, table_name_with_geo_level in enumerate(table_names):
        ...
        if table_name_with_geo_level != "metadata":
            ...
            if table_name not in metadata_table_names:
                column_list = get_column_names_from_table(
                    disk_engine, table_name_with_geo_level)[1:-1]
                print column_list
                table_columns_df = table_columns_df.append(pandas.DataFrame(get_column_list_table_name_array(table_name,
                                                                                                             column_list, long_header_lookup),
                                                                            columns=['long', 'short', 'table_name']))
    progress_bar.finish()

    table_columns_df.set_index('long').to_sql(
        'metadata', disk_engine, if_exists='replace')
