...
def get_column_list_table_name_array(table_name, column_list, long_header_lookup):
  ...
  return map(lambda column_name: [long_header_lookup.get_value(column_name, 'Long'), table_name], column_list)
...
def read_data_for_geo_level_into_database(directory, geo_levels, disk_engine):
  ...
  for geo_level in geo_levels:
    ...
    for i, filename in enumerate(directory_file_list):
      ...
      table_columns_df = table_columns_df.append(pandas.DataFrame(get_column_list_table_name_array(table_name, column_list[1:], long_header_lookup), columns = ['column', 'table_name']))
    bar.finish()
